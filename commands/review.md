...
9. **Spryker idiom & convention checks** — deterministic, grep-detectable conventions that human Spryker reviewers reliably flag and authors reliably fix. These are usually Minor/Nit but are **exempt from the Step 3 signal-budget cap and the "drop lowest-severity" rule** (see Step 3) — report *every* instance, because they are cheap to detect and cheap to fix, and missing them is exactly how a review loses credibility vs. a human reviewer. Each is owned by the pass noted; see `spryker-conventions` skill for the rationale.
   ...
   - **9h — Skip-guard reliability (Pass E).** A test's `markTestSkipped()` (or any conditional skip) driven by an OBSERVED SIDE EFFECT — DOM presence/absence, response shape — rather than the actual controlling state (a feature-flag client call, a config value) can't distinguish "feature intentionally off" from "feature broken." A renderer that silently fails with the flag ON looks identical to flag-OFF and the test reports SKIPPED, not FAILED — CI stays green while the feature is completely broken. Detect: `grep -rn 'markTestSkipped' tests/PyzTest/**/*Cest.php` — for each hit, confirm the skip condition queries the flag/config directly (`getLocator()->featureFlag()->client()->isEnabled(...)`) rather than inferring it from the page (`!$i->see(...)`, an absence-based DOM check). Also confirm the flag is actually exercised in CI (a flag defaulting off with nothing enabling it in the pipeline means every gated test silently skips forever).
...
- Scope:
  ...
  - **Rule-9h/9i idiom checks (report all, exempt from cap):** 9h (skip-guard reliability — a conditional test skip driven by an observed side effect rather than the actual controlling state), 9i (removed test scenarios — a spec/Cest rewrite that drops previously-covered interaction sequences without an equivalent, especially when the underlying implementation became more stateful/shared).
